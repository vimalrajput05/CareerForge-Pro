...
<h1 className="text-3xl font-bold text-gray-900 dark:text-white font-sans tracking-tight bg-gradient-to-r from-violet-600 to-emerald-500 bg-clip-text text-transparent">Career Workshop</h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-6 mb-10">
          {[
            ...
            { id: "ats", label: "ATS Scores", count: atsScores.length }
          ].map((tab) => (
            <div 
              key={tab.id}
              onClick={() => {
              ...
              className={`cursor-pointer p-6 rounded-2xl border-2 transition-all duration-300 ${
                ...
                : "border-transparent bg-white dark:bg-gray-800 shadow-sm hover:border-violet-200 dark:hover:border-violet-800"
              }`}
            >
              <p className="text-sm text-gray-500 dark:text-gray-400 font-medium uppercase tracking-wider">{tab.label}</p>
              <h2 className="text-4xl font-black dark:text-white mt-1">{tab.count}</h2>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                {activeTab === "jobs" && jobs.map(item => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <span className="text-2xl">🏢</span>
                      <div>
                        <p className="font-bold dark:text-white leading-tight">{item.role}</p>
                        <p className="text-xs text-violet-500 font-bold uppercase tracking-tighter mt-1">{item.company}</p>
                      </div>
                    </div>
          ...
          <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-xl border border-violet-100 dark:border-gray-700 h-fit sticky top-10">
            <h3 className="text-xl font-bold mb-4 dark:text-white border-b dark:border-gray-700 pb-2">Details View</h3>
            {selectedItem ? (
              <motion.div initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} key={selectedItem.id}>
                <p className="text-violet-600 font-bold mb-2 uppercase tracking-widest text-[10px]">Active Selection</p>
                <h4 className="text-2xl font-black mb-4 dark:text-white leading-tight">
                  {selectedItem.name || selectedItem.role || selectedItem.title}
                </h4>
                <div className="p-4 bg-slate-50 dark:bg-gray-700/50 rounded-2xl mb-6 min-h-[150px]">
